...
public class UserController extends BaseController {
    ...
    public String view(Model model) {
        ...
        System.out.println("rolesJson："+JSON.toJSONString(roleService.list()));
    ...
    public PageResult<User> page(HttpServletRequest request) {
        ...
        PageResult<User> userPageResult = userService.listPage(pageParam);

        System.out.println("用户管理 JSON.toJSON(userPageResult):"+JSON.toJSON(userPageResult));
...
//        boolean result = userService.removeById(id);
     ...
     * 验证二级密码
     */
    @OperLog(value = "用户管理", desc = "验证二级密码", param = false, result = true)
//    @RequiresPermissions("sys:user:update")
    @ResponseBody
    @RequestMapping("/psw/verifyTwoPassword")
    public JsonResult verifyTwoPassword(Integer id, String twoPassword) {

        System.out.println("verifyTwoPassword id:"+id);

        System.out.println("verifyTwoPassword:"+twoPassword);
        User user = userService.getById(id);
        System.out.println("user："+user);
        System.out.println("user.getTwoPassword()："+user.getTwoPassword());

        if(twoPassword.equals(user.getTwoPassword())){
    ...
    public JsonResult importBatch(MultipartFile file) {
        ...
        try {
            ...
            // 进行非空和重复检查
            sb.append(CoreUtil.excelCheckBlank(list, startRow, 0, 1, 2, 3, 4, 5));
            ...
            for (int i = 0; i < list.size(); i++) {
                ...
                String jobNumber = String.valueOf(objects.get(0));  // 工号
                String username = String.valueOf(objects.get(1));  // 账号
                String password = String.valueOf(objects.get(2));  // 密码
                String nickName = String.valueOf(objects.get(3));  // 用户名
                String sexName = String.valueOf(objects.get(4));  // 性别
                String roleName = String.valueOf(objects.get(5));  // 角色名
